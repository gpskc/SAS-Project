...
proc univariate data = vehicles_tmp; var log_price log_age log_odometer; histogram; inset n mean std min max; run;

/* categorical variables frequency */
proc freq data = vehicles_tmp;
		  tables fuel title_status transmission manufacturer condition cylinders drive size type paint_color / missing;
run;
...
proc sql; update vehicles_tmp set manufacturer = 'other'
		  where manufacturer in ('alfa','alfa-romeo','aston','aston-marti','datsun','ferrari','harley',
								 'harley-davi','hennessey','landrover','land rover','morgan','noble','porche');
quit;
...
proc export data = vehicles_4cat outfile = "E:\pcg180000\final\vehicles_4cat.csv" dbms = csv replace; run;
proc export data = vehicles_9cat outfile = "E:\pcg180000\final\vehicles_9cat.csv" dbms = csv replace; run;
...
proc glm data=vehicles_9cat; class paint_color; model log_odometer=paint_color; lsmeans paint_color/adjust=tukey; run;
